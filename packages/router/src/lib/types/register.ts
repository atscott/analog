 ...
 * Used for O(1) compile-time destination route lookup in navigate() and [routerLink].
 */
export type FileRoutesByFullPath<TRoutes = FileRoutesByPath> = {
  [K in keyof TRoutes as TRoutes[K] extends {
    ...
    ? TFullPath
    : never]: TRoutes[K] extends { route: infer TRoute } ? TRoute : never;
};

/**
 * Automatically maps FileRoutesByPath entries by their unique route `id`.
 * Used for O(1) route lookup in injectRoute(id) and disambiguating layout vs index routes.
 */
export type FileRoutesById<TRoutes = FileRoutesByPath> = {
  [K in keyof TRoutes as TRoutes[K] extends { id: infer TId extends string }
    ? TId
    : never]: TRoutes[K] extends { route: infer TRoute } ? TRoute : never;
};
...
/**
 * Resolves the route map from Register.
 * Follows a 3-tier resolution hierarchy:
 * 1. Explicit `routeMap` defined in `Register`.
 * 2. Inferred from augmented `FileRoutesByPath` via `FileRoutesById`.
 * 3. Permissive fallback to `Record<string, AnyRoute>`.
 */
