...
export interface RouteManifest {
  ...
  /** Full paths defined by more than one file, such as a layout and its index page. */
  sharedFullPaths: Set<string>;
}
...
export function generateRouteManifest(
  ...
  const sharedFullPaths = new Set<string>();
  ...
  // Layout routes are a file and a folder with the same name, so users.page.ts
  // wraps users/[id].page.ts but not users.[id].page.ts. Folders from all
  // pages dirs share one tree.
  const layoutByKey = new Map<string, string>();
  for (const filename of prioritizedFilenames) {
    const key = toRouteFileKey(filename);
    if (!filename.endsWith('.md') && !layoutByKey.has(key)) {
      layoutByKey.set(key, filename);
    }
  }
  ...
  for (const filename of prioritizedFilenames) {
    ...
    const segments = toRouteFileKey(filename).split('/');
    const layouts: string[] = [];
    for (let i = 1; i < segments.length; i++) {
      const layout = layoutByKey.get(segments.slice(0, i).join('/'));
  ...
  for (const route of routes) {
    ...
    if (routeByFullPath.get(route.fullPath) !== route) {
      sharedFullPaths.add(route.fullPath);
    }
  }
...
/** A route file's path within its pages directory, e.g. `users/[id]`. */
function toRouteFileKey(filename: string): string {
  return filename.replace(
    /^.*?[\\/](?:routes|pages|content)[\\/]|\.page\.(?:js|ts|analog|ag)$|\.(?:ts|md|analog|ag)$/g,
    '',
  );
}
...
/** Import specifiers for a page module's typed exports. */
export interface RouteModuleImports {
  routeMeta?: string;
  load?: string;
}
 ...
 *
 * `resolveModuleImports` adds `routeMeta` and `load` types for pages whose
 * URL is defined by a single file, along with the `routeMeta` of the layouts
 * that wrap them. URLs shared by several files, such as a layout and its
 * index page, are ambiguous and only get params and query types.
 */
export function generateRouteTableDeclaration(
  manifest: RouteManifest,
  resolveModuleImports?: (filename: string) => RouteModuleImports | undefined,
): string {
  const lines: string[] = [];
  ...
  const importType = (specifier: string, name: string) =>
    `typeof import(${JSON.stringify(specifier)}).${name}`;
  ...
  for (const route of manifest.canonicalByFullPath.values()) {
    ...
    if (
      ...
      route.kind === 'page' &&
      !manifest.sharedFullPaths.has(route.fullPath)
    ) {
      const layoutRouteMeta = route.layouts.flatMap((layout) => {
        const specifier = resolveModuleImports(layout)?.routeMeta;
        return specifier ? [importType(specifier, 'routeMeta')] : [];
      });
      const imports = resolveModuleImports(route.filename);
      if (layoutRouteMeta.length) {
        lines.push(`      layoutRouteMeta: [${layoutRouteMeta.join(', ')}];`);
      }
      if (imports?.routeMeta) {
        lines.push(
          `      routeMeta: ${importType(imports.routeMeta, 'routeMeta')};`,
        );
      }
      if (imports?.load) {
        lines.push(`      load: ${importType(imports.load, 'load')};`);
      }
    }
    lines.push(`    };`);
  }
