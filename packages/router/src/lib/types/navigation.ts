...
import type { NonUndefined, ParseParams, Simplify } from './params';
import type {
  CleanToPath,
  RemoveLastSegment,
  NormalizeRoot,
...
/**
 * Unwraps a route entry from its container object if wrapped under `route`.
 */
export type UnwrapRoute<T> = T extends { route: infer R } ? R : T;
...
> = TPath extends keyof TNavMap
  ? UnwrapRoute<TNavMap[TPath]> extends { types?: { params?: infer P } }
    ? [NonUndefined<P>] extends [never]
      ? ParseParams<TPath>
      : [NonUndefined<P>] extends [unknown]
        ? unknown extends NonUndefined<P>
          ? ParseParams<TPath>
          : NonUndefined<P>
        : NonUndefined<P>
    : ParseParams<TPath>
...
export type RouteQueryParamsForPath<
  ...
  TNavMap = RegisteredNavigationMap,
> = TPath extends keyof TNavMap
  ? UnwrapRoute<TNavMap[TPath]> extends { types?: { queryParams?: infer Q } }
    ? [NonUndefined<Q>] extends [never]
      ? UnwrapRoute<TNavMap[TPath]> extends { queryParams?: infer Q2 }
        ? NonUndefined<Q2>
        : Record<string, any>
      : [NonUndefined<Q>] extends [unknown]
        ? unknown extends NonUndefined<Q>
          ? Record<string, any>
          : NonUndefined<Q>
        : NonUndefined<Q>
    : UnwrapRoute<TNavMap[TPath]> extends { queryParams?: infer Q }
      ? NonUndefined<Q>
      : Record<string, any>
  : Record<string, any>;
...
export type ParentPath<TFrom extends string> = NormalizeRoot<
...
> =
  ...
  | RelativeNavigateOptionsForRoute<
      ...
      '..' | '../',
      RouteParamsForPath<TFrom, TNavMap>,
      RouteParamsForPath<ParentPath<TFrom>, TNavMap>,
      RouteQueryParamsForPath<ParentPath<TFrom>, TNavMap>
    >
  | RelativeNavigateOptionsForRoute<
      TFrom,
      '.' | './',
      RouteParamsForPath<TFrom, TNavMap>,
      RouteParamsForPath<TFrom, TNavMap>,
      RouteQueryParamsForPath<TFrom, TNavMap>
    >;
