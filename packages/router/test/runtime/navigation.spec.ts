...
describe('Runtime Navigation Engine & Angular Router Bridge (M4)', () => {
  ...
  describe('navigate() with RouterTestingHarness', () => {
    ...
    it('should support functional params updater', async () => {
      await harness.navigateByUrl('/users/42/posts/1');
      ...
      expect(router.url).toBe('/users/42/posts/2');
    });

    it('should extract embedded query and hash from path string', async () => {
      await TestBed.runInInjectionContext(async () => {
        await navigate({ to: '/users?tab=info#bio' });
      });

      expect(router.url).toBe('/users?tab=info#bio');
    });
  ...
  describe('injectNavigate() with RouterTestingHarness', () => {
    ...
    it('should navigate with injectNavigate() to parameterized routes', async () => {
